...
Examples:
  ...
  python view_metadata.py  (interactive mode)
    ...
    # Get URLs via resolve_query so exposure numbers come from filenames
    try:
        ...
        url = q.urls[sca_num].get(exp_num)
        if url is None:
        ...
        buf = stream_to_buffer(url, mast_token=mast_token, show_progress=True)
