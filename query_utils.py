...
from streaming_utils import get_MAST_token, group_file_urls, get_exp_num
...
def resolve_query(visit_id, exp_spec=None, data_level=2, sca_spec=None, mast_token=None):
    ...
    raw_urls = group_file_urls(mast_token, visit_id, exp_num='', data_level=data_level)

    if not raw_urls:
        raise ValueError(f"No data found for visit pattern '{visit_id}' at level {data_level}")

    all_scas = sorted(raw_urls.keys())
    ...
    # --- Build per-visit summary and collect filenames from the raw URL lists ---
    # Iterate the raw positional lists so that multiple visits matching a wildcard
    # are each counted separately (same real_exp_num in different visits must not
    # collapse into one dict entry at this stage).
    summary = {}
    matched_files = []
    for sca_num in scas:
        for url in raw_urls[sca_num]:
            if url is None:
                continue
            filename = url.split('%2F')[-1]
            real_exp = get_exp_num(filename)
            if real_exp is None:
                continue
            if exp_list is not None and real_exp not in exp_list:
                continue
            m = re.search(r'r(\d{18,})', url)
            if m:
                vid = m.group(1)
                if vid not in summary:
                    summary[vid] = {'exposures': set(), 'scas': set()}
                summary[vid]['exposures'].add(real_exp)
                summary[vid]['scas'].add(sca_num)
            matched_files.append(filename)

    if not summary:
        raise ValueError(f"No exposures match the requested filter (available exp spec: {exp_spec!r})")

    # Unique real exposure numbers across all matched URLs (for single-visit callers)
    exp_nums = sorted({e for info in summary.values() for e in info['exposures']})

    # --- Build {sca: {real_exp_num: url}} for streaming callers ---
    # For single-visit queries this is unambiguous.  For wildcard queries that
    # matched multiple visits the last writer wins per (sca, exp_num) — callers
    # that stream by exp_num (export_metadata_csv, write_wfi_fits) should always
    # use a specific single-visit visit_id, so this is not a problem in practice.
    filtered_urls = {}
    for s in scas:
        filtered_urls[s] = {}
        for url in raw_urls[s]:
            if url is None:
                continue
            real_exp = get_exp_num(url.split('%2F')[-1])
            if real_exp is not None and (exp_list is None or real_exp in exp_list):
                filtered_urls[s][real_exp] = url
    ...
    if summary:
        ...
        print("  (No visit IDs could be parsed from URLs)")
